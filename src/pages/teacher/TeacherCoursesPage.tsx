...
export default function TeacherCoursesPage() {
  ...
  const handleCreateCourse = async (data: Omit<Course, 'id' | 'createdAt'>) => {
    // Ensure the teacher is set to current user
    const result = await createCourse({ ...data, teacherId: user!.id });
    if (result.success) {
      loadData();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl font-bold">Mis Cursos</h1>
          <p className="text-muted-foreground mt-1">Gestiona los cursos que impartes</p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            ...
            {courses.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">No tienes cursos creados. ¡Crea el primero!</div>
            )}
          </div>
        </CardContent>
      </Card>
